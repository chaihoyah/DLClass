...
class nn_linear_layer:
    ...
    def forward(self,x):
        x = np.array(x)
        dot_res = np.dot(self.W,x.T).T
        for i in range(len(x)):
            dot_res[i] = dot_res[i] + self.b[:,0]
        return dot_res
    ...
    def backprop(self,x,dLdy):
        data_num = len(x)
        dLdW = (np.dot(x.T,dLdy)).T/data_num
        dLdb = np.sum(dLdy,axis=0)/data_num
        dLdx = np.dot(dLdy,self.W)/data_num
...
class nn_activation_layer:
    ...
    def backprop(self,x,dLdy):
        data_num = len(x)
        ones = np.ones((data_num,len(x[0])))
        forward_res = self.forward(x)
        res = ones-forward_res
        final_res = res*forward_res*dLdy
        return final_res
...
class nn_softmax_layer:
    ...
    def backprop(self,x,dLdy):
        forward_res = self.forward(x)
        data_num = len(x)
        res_one = np.array([forward_res[i][0]*forward_res[i][1] for i in range(data_num)])*dLdy[:,0]
        res_two = np.array([forward_res[i][0]*forward_res[i][1] for i in range(data_num)])*dLdy[:,1]
        return np.array([res_one,res_two]).T
...
class nn_cross_entropy_layer:
    ...
    def forward(self,x,y):
        data_num = len(x)
        res = [np.log(x[i][0]) if y[i] == 0 else np.log(x[i][1]) for i in range(data_num)]
        b = np.array([0 if y[i]==1 else 1 for i in range(data_num)])
        for i in range(data_num):
            if b[i] == 1:
                res.append(np.log(x[i][0]))
            else:
                res.append(np.log(x[i][1]))

        return -1*(np.sum(res)/data_num)
    ...
    def backprop(self,x,y):
        data_num = len(x)
        b = np.array([0 if y[i]==1 else 1 for i in range(data_num)])
        dLdp_one = np.array([((x[i][0]-b[i])/(x[i][0]*x[i][1])) for i in range(data_num)])
        dLdp_two = np.array([((-1+x[i][1]+b[i])/(x[i][0]*x[i][1])) for i in range(data_num)])
        return np.array([dLdp_one,dLdp_two]).T
